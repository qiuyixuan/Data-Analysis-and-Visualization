...
class Analysis:
    ...
    def mean(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        sumarr = np.sum(self.data.select_data(headers, rows), axis = 0)
        numrows = len(self.data.select_data(headers, rows))    
        return sumarr / numrows
    ...
    def var(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        meanarr = self.mean(headers, rows)
        diffarr = self.data.select_data(headers, rows) - meanarr
        sqrarr = diffarr**2
        sumarr = np.sum(sqrarr, axis = 0)
        numrows = len(self.data.select_data(headers, rows))

        return sumarr / (numrows - 1)
    ...
    def scatter(self, ind_var, dep_var, title=None):
        ...
        '''
        x = np.squeeze(self.data.select_data(ind_var))
        y = np.squeeze(self.data.select_data(dep_var))
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        for row_idx in range(len(data_vars)):
            for col_idx in range(len(data_vars)):
                axes[row_idx, col_idx].scatter(self.data.select_data(data_vars[col_idx]), self.data.select_data(data_vars[row_idx]))             
                if row_idx == len(data_vars) - 1:
                    axes[len(data_vars) - 1, col_idx].set_xlabel(data_vars[col_idx])
                if col_idx == 0:    
                    axes[row_idx, 0].set_ylabel(data_vars[row_idx]) 
